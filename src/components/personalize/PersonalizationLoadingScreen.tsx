...
import { Brain, User, Target, Scale, CheckCircle2, SlidersHorizontal } from "lucide-react";

interface PersonalizationLoadingScreenProps {
  progress: number;
  message: string;
}

export function PersonalizationLoadingScreen({ progress, message }: PersonalizationLoadingScreenProps) {
  // Determine icon based on progress range
  let Icon = Brain;
  if (progress < 25) Icon = User;
  else if (progress < 45) Icon = SlidersHorizontal;
  else if (progress < 70) Icon = Scale;
  else if (progress < 95) Icon = Target;
  else Icon = CheckCircle2;

  return (
    <motion.div
      key="personalization-loading-screen"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
